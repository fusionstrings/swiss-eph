...
try {
  ...
  if (filesLoaded > 0) {
    console.log(`deno | wasmbuild | direct | ${filesLoaded} files loaded.`);
  }
} catch (e) {
...
if (moshierVal === swissVal) {
  ...
  console.log(
    "PASS: Direct wasmbuild Swiss mode verification (Swiss != Moshier).",
  );
...
console.log(
  `deno | wasmbuild | direct_wasm | swiss: Sun longitude = ${
    finalVal.toFixed(6)
  }°`,
);
